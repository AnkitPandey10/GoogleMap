...
class EditFoodTrucksPage extends StatefulWidget {
  EditFoodTrucksPage({Key key, this.colId, this.foodType, this.foodDetails, this.openTime, this.closeTime, this.address ,this.latitude, this.longitude}) : super(key: key);
  final dynamic colId;
...
class _MyHomePageState extends State<EditFoodTrucksPage>{
  ...
  void initState(){
    super.initState();
    setState(() {
      foodTypeController.text = widget.foodType;
      foodDetailsController.text = widget.foodDetails;
      latitudeController.text = widget.latitude.toString();
      longitudeController.text = widget.longitude.toString();
      addressController.text = widget.address;
      openingTimeController.text = widget.openTime;
      closingTimeController.text = widget.closeTime;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: new Text('Edit Food Truck',style: TextStyle(color: whiteBack,fontFamily: "Roboto-Regular",fontSize: 18.0,letterSpacing: 0.0),),
        centerTitle: true,
        leading: Builder(
  ...
  void updateFoodTruckData() async {
    ...
    int id = await foodTruckDatabaseHelper.updateFoodTruck(row,widget.colId);
    print('inserted row id: $id');
    print(await foodTruckDatabaseHelper.queryAllRowsFoodTruck());
    SampleToast.updateTruck();
    Navigator.pop(context);
  }
}